...
image = cv2.imread("image/test_input.png")
(h, w, _) = image.shape
opencv_resize(image, w, h)
...
cv2.imwrite('./image/out-edsr_x4.bmp', out_img)
...
while True:
    ret, frame = cap.read()
    if ret == False or frame_count > 1:
        break;
    out_frame = sr.upsample(frame)
    cv2.imwrite('./out/edsrx4_%03d.bmp' % frame_count, out_frame)
